...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	priorityReviewRepository := repository.NewPriorityReviewRepository(db)
	...
	defectFindingService := service.NewDefectFindingService(defectFindingRepository, priorityReviewRepository, priorityDecisionRepository, securityService)
	priorityDecisionService := service.NewPriorityDecisionService(priorityDecisionRepository, securityService)
	priorityReviewService := service.NewPriorityReviewService(priorityReviewRepository, priorityDecisionRepository, securityService)
	...
	priorityReviewHandler := handler.NewPriorityReviewHandler(priorityReviewService)
	...
	priorityReviewHandler.Register(api)
