...
export function statusTone(status: string): 'success' | 'warning' | 'danger' | 'neutral' {
  if (/approved|accepted|released|completed|signed|closed|pass|ready|online|cleared|succeeded|maintained/.test(status)) return 'success';
  if (/failed|rejected|critical|scrap|discard|revoked|urgent|escalated/.test(status)) return 'danger';
